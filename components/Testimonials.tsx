    ...
    <section className="py-24 bg-bg-primary relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start">
          ...
          >
            {/* Badge */}
            <span className="inline-block px-4 py-1.5 border border-accent/40 rounded-full text-[11px] uppercase tracking-[0.15em] text-accent mb-8">
              Stories of Success
            </span>

            {/* Heading */}
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-text-primary leading-[1.1] mb-10">
              Hear it{" "}
              <span className="text-text-muted italic font-normal">from</span>
              <br />
              our clients
            </h2>
            ...
            <Link
              href="/work"
              className="inline-flex items-center px-6 py-3 border border-border rounded-full text-sm text-text-primary hover:bg-accent/5 transition-colors"
            >
              Read more stories
            </Link>
          </motion.div>
              ...
              >
                {visibleTestimonials.map((testimonial, index) => (
                  <motion.div
                    ...
                    className="bg-bg-card border border-border rounded-2xl p-8 relative overflow-hidden group shadow-lg shadow-black/5"
                  >
                    ...
                    {/* Quote */}
                    <p className="text-text-secondary text-[15px] leading-relaxed mb-6 relative z-10">
                      {testimonial.quote}
                    </p>
              ...
              {Array.from({ length: Math.ceil(testimonials.length / 2) }).map(
                (_, index) => (
                  <button
                    ...
                    className={`h-1.5 rounded-full transition-all duration-300 ${index === currentPair
                      ? "w-8 bg-accent/60"
                      : "w-1.5 bg-text-muted/20 hover:bg-text-muted/30"
                      }`}
